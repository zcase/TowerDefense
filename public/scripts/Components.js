    ...
    function Bullet(spec) {
        ...
        that.update = function (creeps, particleSystems) {
            ...
            if (that.type === 'missile'){
               var missileTail = effects.ParticleSystem({
                    image: 'images/Fire-a.png',
                    center: { x: xPos, y: yPos },
                    speed: { mean: 50, std: 25 },
                    rotation: spec.rotation,
                ...
                particleSystems.push(missileTail);
            }
            ...
            for (var i = 0; i < creeps.length; i++) {
                if (distanceCheck(that, creeps[i])) {
                    ...
                    if (that.type !== 'freeze' && that.type !== 'bomb') {
                        ...
                        that.attackDistance *= 9;
                        for (var z = 0; z < creeps.length; z++) {
                            if (distanceCheck(that, creeps[z])) {
                                var xPos = ((that.x * 20));
                                var yPos = ((that.y * 20));
                                var hitCreepBombA = effects.ParticleSystem({
                                    image: 'images/Fire-a.png',
                                    center: { x: xPos, y: yPos },
                                    speed: { mean: 30, std: 5 },
                                    rotation: spec.rotation,
                                ...
                                var hitCreepBombB = effects.ParticleSystem({
                                    image: 'images/smoke.png',
                                    center: { x: xPos, y: yPos },
                                    speed: { mean: 30, std: 5 },
                                    rotation: spec.rotation,
                                ...
                                for (var i = 0; i < 1000; i++){
                                    hitCreepBombA.create();
                                    hitCreepBombB.create();
                                }
                                particleSystems.push(hitCreepBombA);
                                particleSystems.push(hitCreepBombB);
                                creeps[z].health -= that.damage;
                            }
        ...
        that.render = function () {
            if (that.type !== 'missile' && that.type !== 'bomb' && that.hitCreep !== true) {
